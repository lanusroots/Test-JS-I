...
function redondearNumero(num) {
  ...
  let entero;
  if (num >= 0) {
    entero = ~~(num + 0.5); // Redondeo hacia arriba
  } else {
    entero = ~~(num - 0.5); // Redondeo hacia abajo
  }
  return entero;
}
...
function redondearHaciaArriba(num) {
  ...
  // Tu código:
  let entero = parseInt(num, 10); // Obtengo la parte entera del número
  
  if (num > entero) {
    return entero + 1; // Si hay parte decimal, redondeo hacia arriba sumando 1
  } else {
    return entero; // Si no hay parte decimal, el número ya está redondeado
  }
}
